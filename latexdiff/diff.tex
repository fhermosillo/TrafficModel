...
%DIF DEL D:/CINVESTAV/Doctorado/Cuatrimestre/7/Dr Deni/TrafficModel/tmp.old.tex                 Wed Oct 28 17:06:27 2020
%DIF ADD D:/CINVESTAV/Doctorado/Cuatrimestre/7/Dr Deni/TrafficModel/Vehicle Traffic Model.tex   Wed Oct 28 17:05:34 2020
...
where $\mathten{B} \in \mathbb{R}^{W\times H \times D \times Time}$ \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{represents the background, which can be modeled as }\DIFaddend a low-rank tensor \DIFdelbegin \DIFdel{which capture low-frequency components the video, i.e., the background, }\DIFdelend \DIFaddbegin \DIFadd{that capture the lowest frequency component of the video, }\DIFaddend while $\mathten{F} \in \mathbb{R}^{W\times H \times D \times Time}$ \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{represents the foreground modeled as }\DIFaddend a sparse tensor \DIFdelbegin \DIFdel{that contains motion information on the video, in other words it represents the foreground}\DIFdelend \DIFaddbegin \DIFadd{containing motion information of the video}\DIFaddend . Note that in Figure \ref{fig:tsvmodel} exists another tensor denoted by $\mathten{F}_m \in \mathbb{R}^{W\times H \times Time}$, which is the \DIFdelbegin \DIFdel{binary }\DIFdelend mask of $\mathten{F}$ \DIFaddbegin \DIFadd{that is obtained by applying some binary operations on $\mathten{F}$}\DIFaddend .
...
For this model, there exist some methods and algorithms that successfully decompose a traffic surveillance video into the background and foreground components such as the Gaussian Mixture Model \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin [\DIFadd{X}]\DIFadd{, }\DIFaddend Robust Principal Component Analysis \DIFaddbegin [\DIFadd{X}]\DIFadd{, and the pixel-entropy }[\DIFadd{X}] \DIFaddend (see [X] for an extensive review on this decomposition). In this work, we will use \DIFdelbegin \DIFdel{a modified version of }\DIFdelend the Tensor Robust Principal Component Analysis or t-RPCA in short, originally proposed by Lu, C., et. al., [X] to achieve such decomposition.

\subsection{Moving Vehicle Traffic Tensor Modeling}

From the foreground tensor $\mathten{F}$, information about moving vehicles can be extracted such as their trajectory, geometry, kinematic or color information, which can be used later at a particular \DIFdelbegin \DIFdel{stage }\DIFdelend \DIFaddbegin \DIFadd{task }\DIFaddend of a VTS system. However, due to the high-volume of data and multimodality induced by \DIFdelbegin \DIFdel{multi-stage }\DIFdelend \DIFaddbegin \DIFadd{multi-task }\DIFaddend VTS systems, a one-mode representation results to be not enough to exploit interactions among \DIFdelbegin \DIFdel{stages}\DIFdelend \DIFaddbegin \DIFadd{tasks}\DIFaddend .

To tackle the shortcomings of one-mode models, we proposed to arrange and group vehicle information as a high-order structure $\mathten{T}^{(N)} \in \mathbb{R}^{I_1 \times \cdots \times I_N}$\DIFaddbegin \DIFadd{, here }\DIFaddend called Vehicle Traffic Feature (VTF) tensor\DIFaddbegin \DIFadd{, }\DIFaddend to model the vehicle behavior of a feature model over multiple \DIFdelbegin \DIFdel{stages}\DIFdelend \DIFaddbegin \DIFadd{tasks}\DIFaddend . Even though there is a vast set of features which can be extracted from $\mathten{F}$, here we will focus on the geometric feature model, for which we will assume that the behavior of any feature over time can be described by polynomial functions.
...
\DIFaddend Similar to linear transformations, multilinear transformations also provide us different properties or structures [] that can be exploited according to the problem faced, hence the use of a particular transformation must be application dependent. Here, we will seek for those functions which provide a higher-order representation of data while inducing well-known intrinsic structures. The first desired property will be needed for both preserving linear mixtures of the source space and \DIFdelbegin \DIFdel{avoiding }\DIFdelend \DIFaddbegin \DIFadd{avoids }\DIFaddend to introduce non-separable terms [], while the second property \DIFdelbegin \DIFdel{allows }\DIFdelend \DIFaddbegin \DIFadd{will allow us }\DIFaddend to enjoy useful properties to be exploited in TD. For that purpose, we employed two deterministic multilinear transformations called Hankelization and L{\"o}wnerization \DIFdelbegin %DIFDELCMD < []%%%
\DIFdelend \DIFaddbegin \DIFadd{(see Definition \ref{def:hankeltrans} and \ref{def:loewnertrans})}\DIFaddend , which map the source space into a higher-order structure that have approximately \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend intrinsic low multilinear-rank. On the other hand, these transformations enjoy some useful properties for data modeling, for example, Hankel structures \DIFaddbegin \DIFadd{(Definition \ref{def:hankel}) }\DIFaddend are known for representing exponential polynomials [], while L{\"o}wner structures \DIFaddbegin \DIFadd{(Definition \ref{def:loewner}) }\DIFaddend show a very close relationship with rational functions [], so that they can be used to model and approximate a wide variety of functions.

\DIFaddbegin \begin{Definition}[Hankel Structure]
	\label{def:hankel}
\end{Definition}
...
\begin{Definition}[Hankelization Transformation]
	...
	\DIFaddend The $K$th-order Hankelization is a transformation that maps \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{any }\DIFaddend vector $\mathvec{x}\in\mathbb{R}^N$ into a $K$\DIFdelbegin \DIFdel{-order }\DIFdelend \DIFaddbegin \DIFadd{th-order }\DIFaddend tensor $\mathten{H}^{(K)} \in\mathbb{R}^{I_1 \times \cdots \times I_K}$ called Hankel tensor with constant anti-diagonal hyperplanes as Equation \ref{eq:hankelization} shows, where $\mathcal{H}$ \DIFdelbegin \DIFdel{represents }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend the Hankelization transformation, and \DIFdelbegin \DIFdel{$N = I_1 + \cdots + I_K - K + 1$.
	On the other hand, the }\DIFdelend \DIFaddbegin \DIFadd{$N = \sum_{k=1}^{K} {I_k} - K + 1$.
	%DIF >  Into a Kth-order Hankel tensor with constant ...
	%DIF >  N = I_1 + ... + I_K - K + 1
	}
...
\begin{Definition}[L{\"o}wnerization Transformation]
	...
	\DIFadd{The }\DIFaddend $K$th-order L{\"o}wnerization is a transformation that maps a vectorized function \DIFdelbegin \DIFdel{$\mathvec{f}\in\mathbb{R}^N$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathvec{x}(\boldsymbol{\phi})\in\mathbb{R}^N$ }\DIFaddend evaluated at $N$ points $\boldsymbol{\phi}\in\mathbb{R}^{N}$ into a $K$\DIFdelbegin \DIFdel{-order }\DIFdelend \DIFaddbegin \DIFadd{th-order }\DIFaddend tensor $\mathten{L}^{(K)} \in \mathbb{R}^{I_1 \times \cdots \times I_K}$ called L{\"o}wner tensor such that each entry is defined as Equation \ref{eq:loewnerization} shows, where $\mathcal{L}$ \DIFdelbegin \DIFdel{represents }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend the $K$th-order L{\"o}wnerization transformation, while $\mathvec{f}^{(p)}$ is the $p$th subset of $\boldsymbol{\phi}$, which holds that  \DIFdelbegin \DIFdel{$\mathvec{f}^{(p)} \bigcap \mathvec{f}^{(q)} = \boldsymbol{\emptyset}$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathvec{f}^{(p)} \bigcap \mathvec{f}^{(q)} = \boldsymbol{\emptyset} \:\:\forall p \neq q$, }\DIFaddend and $\boldsymbol{\phi}=\bigcup_{k=1}^{K} {\mathvec{f}^{(k)}}$.
	%DIF >  Into a Kth-order Loewner tensor such that ...
	%DIF >  the pth disjoint subset of Phi, i.e., f^(p) Intersection ...

	\begin{equation}
	\DIFdelbegin \DIFdel{\mathten{H}}\DIFdelend \DIFaddbegin \DIFadd{\mathten{L}}\DIFaddend ^{(K)} = \DIFdelbegin \DIFdel{\mathcal{H}}\DIFdelend \DIFaddbegin \DIFadd{\mathcal{L}}\DIFaddend (\mathvec{x}) : \DIFdelbegin \DIFdel{h^{(K)}}\DIFdelend \DIFaddbegin \DIFadd{\ell}\DIFaddend _{i_1,\cdots,i_K} = \DIFdelbegin \DIFdel{x_{i_1 + \cdots + i_K - K + 1} %DIF <  \mathten{H}^{(K)}_{:,:,i_3,\cdots,i_K} = \mathcal{H}(\mathten{H}^{(K-1)}_{:,i_3,\cdots,i_K}), \,
}%DIFDELCMD < \label{eq:hankelization}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{\sum_{k=1}^{K}{ \frac{x_{\phi_{k}^{(k)}}}{\prod_{p=1,p\neq k}^{K}{(f_{k}^{(k)} - f_p^{(p)})}}}
...
\DIFdel{\mathten{L}^{(K)} = \mathcal{L}(\mathvec{x}) : \ell_{i_1,\cdots,i_K} = \sum_{k=1}^{K}{ \frac{x_{\phi_{k}^{(k)}}}{\prod_{m=1,m\neq k}^{K}{(f_{k}^{(k)} - f_m^{(m)})}}}
...
Although we can extract a bunch set of multidimensional information to our VFT tensor in different ways, we will focus our attention on two fundamental problems: the pattern recognition \DIFaddbegin \DIFadd{for ASDW }\DIFaddend and the use of multilinear transformations to enforce low-rank assumptions on tensor models. We start by analyzing the pattern recognition \DIFaddbegin \DIFadd{of ASDW }\DIFaddend followed by low-rank models under multilinear transformations.

\subsubsection{Pattern Recognition \DIFaddbegin \DIFadd{for ASDW}\DIFaddend }
Through the VTF model we can exploit multi-mode \DIFdelbegin \DIFdel{correlations }\DIFdelend \DIFaddbegin \DIFadd{relations }\DIFaddend by Tensor Decompositions (TD), which provide a powerful analytical tool for dealing with multidimensional statics. Common TD employed include the CP-decomposition, Tucker model, HoSVD, and an SVD-based on the t-product called t-SVD, where the choice of a particular decomposition must be application dependent. The CP model is generally used for latent factor extraction, while the Tucker model to uncover hidden pattern in data. On the other hand, the choice of the t-SVD is more suitable when dealing with oriented-tensors.

\subsubsection{\DIFaddbegin \DIFadd{High-order }\DIFaddend Low-rank \DIFdelbegin \DIFdel{by }\DIFdelend \DIFaddbegin \DIFadd{Model under }\DIFaddend Multilinear Transformations}
...
A classical model employed in many real-world applications is the (multidimensional) low-rank model (LRM) []\DIFaddbegin \DIFadd{. The model assumes that data have approximately low intrinsic dimensionality, e.g. lie either on some low-dimensional subspace or manifold }[\DIFadd{15}\DIFaddend ,\DIFdelbegin \DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{46}]\DIFadd{, or is sparse in some basis }[\DIFadd{13}]\DIFadd{. Formally, LRM }\DIFaddend states that any measurement tensor $\mathten{X}\in\mathbb{R}^{I_1\times \cdots I_N}$ can be \DIFdelbegin \DIFdel{approximated as the sum of two components, }\DIFdelend \DIFaddbegin \DIFadd{decomposed as $\mathten{X} = \mathten{L} + \mathten{N}$, where $\mathten{L}\in\mathbb{R}^{I_1\times \cdots I_N}$ is }\DIFaddend a low-rank tensor\DIFdelbegin \DIFdel{$\mathten{L}\in\mathbb{R}^{I_1\times \cdots I_N}$, and a small dense noise or approximation error tensor$\mathten{N}\in\mathbb{R}^{I_1\times \cdots I_N}$, i.e., $\mathten{X} = \mathten{L} \simeq \mathten{N}$. This model }\DIFdelend \DIFaddbegin \DIFadd{, and $\mathten{N}\in\mathbb{R}^{I_1\times \cdots I_N}$ a residual tensor, a problem that }\DIFaddend is posed as an optimization problem, \DIFdelbegin \DIFdel{where in its simplest form, it can be formulated }\DIFdelend \DIFaddbegin \DIFadd{commonly solved }\DIFaddend by minimizing the \DIFdelbegin \DIFdel{Frobenius norm }\DIFdelend \DIFaddbegin \DIFadd{norm of $\mathten{N}$ }\DIFaddend as Equation \ref{eq:lowrankmodel} shows.
...
However, the brittleness of LRM with respect to grossly corrupted observations, often causes non optimal solutions []. For this reason, it is very common to replace \ref{eq:lowrankmodel} by \DIFdelbegin \DIFdel{its robust version $\mathten{X} \simeq \mathten{L} + \mathten{S} + \mathten{N}$ }\DIFdelend \DIFaddbegin \DIFadd{the robust model $\mathten{X} = \mathten{L} + \mathten{S} + \mathten{N}$, }\DIFaddend called Robust LRM (RLRM), which is \DIFdelbegin \DIFdel{commonly }\DIFdelend formulated as the minimization problem of Equation \ref{eq:lowrankmodel2}, where $\left\lVert \cdot \right\rVert_\star$ denotes the tensor nuclear norm, $\left\lVert \cdot \right\rVert_1$ the $L_1$-norm, and $\mathten{S}\in\mathbb{R}^{I_1\times \cdots I_N}$ \DIFdelbegin \DIFdel{a sparse component }\DIFdelend \DIFaddbegin \DIFadd{is a sparse tensor }\DIFaddend which models grossly corruptions. \DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{It should be notice that this }\DIFaddend decomposition model can be consider as a generalization of Equation \ref{eq:lowrankmodel}, since in the case of free-corruption, Equation \ref{eq:lowrankmodel2} \DIFdelbegin \DIFdel{is equivalent }\DIFdelend \DIFaddbegin \DIFadd{reduces }\DIFaddend to \ref{eq:lowrankmodel}.
...
\DIFadd{Reduccion de la complejidad de sistemas.
}

\DIFadd{APARTE DE ESTE MODELO, REDACTAR CON EL ARTICULO DE "STRUCTURED TOTAL LEAST SQUARES" ASÍ COMO KERNEL PCA.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Experiments}
\DIFaddbegin \subsection{\DIFadd{Test Evaluation}}
\DIFaddend 

\DIFaddbegin \subsection{\DIFadd{Modeling and Approximation of Functions}}
\DIFadd{Reduccion de la complejidad de sistemas
}


\DIFaddend %This section may be divided by subheadings. It should provide a concise and precise description of the experimental results, their interpretation as well as the experimental conclusions that can be drawn.
%\begin{quote}
...
\DIFdelbegin \DIFdel{Authors should discuss the results and how they can be interpreted in perspective of previous studies and of the working hypotheses.The findings and their implications should be discussed in the broadest context possible.Future research directions may also be highlighted}\DIFdelend \DIFaddbegin \DIFadd{Reconocimiento de patrones para ASDW, a partir de los cuales .}\DIFaddend .\DIFaddbegin \DIFadd{.
}\DIFaddend 

\DIFaddbegin \DIFadd{Conexion de tensores hankel con la complejidad de sistenas multidimensionales multicanal, la complejidad de la tarea de clasificacion o del input space se ve reducida.
}

\DIFadd{Hankel aproxima funciones exponenciales del tipo $a^n$, las caracteristicas geometricas tienen este comportamiento y pueden ser aproximadas por tensores hankel, explotamos el bajo rango que presenta esta transformacion.
}

\DIFadd{Loewner aproxima funciones racionales, sin embargo existe una relacion entre hankel y loewner, de esta manera podemos emplearlo del mismo modo para modelar las caracteristicas geometricas empleando funciones racionales, en conjunto con t-RPCA, se logra una aproximacion mas robusta y exacta, pero no presenta bajo rango. Estudiar las propiedades de estos tensores.
}

%DIF > Authors should discuss the results and how they can be interpreted in perspective of previous studies and of the working hypotheses. The findings and their implications should be discussed in the broadest context possible. Future research directions may also be highlighted.
...
\\
\\
TRABAJO FUTURO: 1. USO DE \DIFdelbegin \DIFdel{TRANSFORMACIONES MULTI-LINEALES? ESTOCASTICAS, }\DIFdelend \DIFaddbegin \DIFadd{OTRAS TRANSFORMACIONES MULTILINEALES COMO ESTOCASTICAS, SEGMENTATION, }\DIFaddend ASI  COMO NO LINEALES \DIFaddbegin \DIFadd{(KERNELS)}\DIFaddend . 2. USO DE LAS DESCOMPOSICIONES TENSORIALES CON RESTRICCIONES ESTRUCTURADAS (HANKEL, LOEWNER, ETC), ASI COMO OTRAS RESTRICCIONES (NO NEGATIVIDAD) \DIFaddbegin \DIFadd{STRUCTURED TENSOR DECOMPOSITIONS. 3. EMPLEO DEL MODELADO DE LOS DATOS PARA LA DETECCION DE OCLUSIONES}\DIFaddend .
