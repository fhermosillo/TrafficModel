...
%DIF DEL D:\CINVESTAV\Doctorado\Cuatrimestre\7\Dr Deni\TrafficModel/tmp.old.tex                 Mon Oct 12 18:58:13 2020
%DIF ADD D:\CINVESTAV\Doctorado\Cuatrimestre\7\Dr Deni\TrafficModel/Vehicle Traffic Model.tex   Mon Oct 12 18:55:05 2020
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Equation \ref{eq:hankelization}, }\DIFaddend where $\mathcal{H}$ represents the Hankelization transformation, \DIFdelbegin \DIFdel{$\mathten{I}$ a F-diagonal tensor, }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend $N = I_1 + \cdots + I_K - K + 1$.  \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend On the other hand, the $K$th-order L{\"o}wnerization is a transformation that maps a vector $\mathvec{x}\in\mathbb{K}^N$ into a $K$-order tensor $\mathten{L}\in\mathbb{K}^{I_1 \times \cdots \times I_K}$ called L{\"o}wner tensor \DIFdelbegin \DIFdel{with entries defined by Equation \ref{eq:loewnerization} . L}%DIFDELCMD < {%%%
\DIFdel{\"o}%DIFDELCMD < }%%%
\DIFdel{wner matrices }%DIFDELCMD < [%%%
\DIFdel{148}%DIFDELCMD < ] %%%
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{such that each entry is defined as Equation \ref{eq:loewnerization} shows, }\DIFaddend where $\phi_{i_k}^{(k)}$ represents ASASASAS, while $f_{i_p}^{(p)}$ BLABLABLA. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend The above ideas can also be extended to higher-order tensors in a straightforward way.
\DIFaddbegin 

\begin{equation}
\DIFadd{\mathten{H}^{(K)} = \mathcal{H}(\mathvec{x}) : \mathten{H}^{(K)}_{:,:,i_3,\cdots,i_K} = \mathcal{H}(\mathten{H}^{(K-1)}_{:,i_3,\cdots,i_K}), \, h^{(K)}_{i_1,\cdots,i_K} = x_{i_1 + \cdots + i_K - K + 1}
