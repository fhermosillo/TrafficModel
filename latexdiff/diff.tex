...
%DIF DEL D:/CINVESTAV/Doctorado/Cuatrimestre/7/Dr Deni/TrafficModel/tmp.old.tex                 Fri Oct 30 09:32:51 2020
%DIF ADD D:/CINVESTAV/Doctorado/Cuatrimestre/7/Dr Deni/TrafficModel/Vehicle Traffic Model.tex   Fri Oct 30 01:01:09 2020
...
Traditional vehicle traffic models treat data as a one-dimensional features vector \DIFaddbegin []\DIFaddend , to later be used to capture the internal correlation of historical data. However, vehicle traffic data is multi-mode by experiments, e.g., features mode, time mode, vehicle class mode, occlusion mode, among others, therefore, the current models turn out to be inadequate to capture these multidimensional interactions. The proposal of a multidimensional model will preserve the multi-mode nature of data, while the use of tensor methods such as decompositions, will help to better capture correlations among all modes.

\subsection{Traffic Surveillance Video Modeling}

Given a traffic surveillance video modeled as a four-order tensor $\mathten{V}_\tau \in \mathbb{R}^{W\times H \times D \times Time}$ of \DIFaddbegin \DIFadd{width and height }\DIFaddend $W \times H$ resolution \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend a duration of $\tau$ seconds, and where each pixel is mapped in some color-space \DIFdelbegin \DIFdel{of dimension $D$ (e.g., grayscale, RGB)}\DIFdelend \DIFaddbegin \DIFadd{in $\mathbb{R}^D$, such as binary, grayscale, and RGB}\DIFaddend . Then, we will assume that there exist some tensor decomposition model such that \DIFdelbegin \DIFdel{the following Equation }\DIFdelend \DIFaddbegin \DIFadd{Equation \ref{eq:tsvmodel} }\DIFaddend holds (see Figure \ref{fig:tsvmodel}):
...
where $\mathten{B} \in \mathbb{R}^{W\times H \times D \times Time}$ represents the background, which can be modeled as a low-rank tensor that capture the lowest frequency component of the video, while $\mathten{F} \in \mathbb{R}^{W\times H \times D \times Time}$ represents the foreground modeled as a sparse tensor containing motion information of the video. Note that in Figure \ref{fig:tsvmodel} exists another tensor denoted by $\mathten{F}_m \in \mathbb{R}^{W\times H \times Time}$, which is the mask of $\mathten{F}$ that is obtained by applying some binary operations on \DIFdelbegin \DIFdel{$\mathten{F}$}\DIFdelend \DIFaddbegin \DIFadd{it}\DIFaddend .
...
\DIFdelbegin %DIFDELCMD < \begin{Definition}[Hankel Structure]
%DIFDELCMD < 	\label{def:hankel}
%DIFDELCMD < \end{Definition}
%DIFDELCMD < 

%DIFDELCMD < \begin{Definition}[L{\"o}wner Structure]
%DIFDELCMD < 	\label{def:loewner}
%DIFDELCMD < \end{Definition}
%DIFDELCMD < 	

%DIFDELCMD < %%%
\DIFdelend \begin{Definition}[Hankelization Transformation]
...
Although we can extract a bunch set of multidimensional information to our VFT tensor in different ways, we will focus our attention on two fundamental problems: the pattern recognition for ASDW and the use of multilinear transformations to enforce low-rank assumptions on tensor models. We start by analyzing the pattern recognition of \DIFdelbegin \DIFdel{ASDW }\DIFdelend \DIFaddbegin \DIFadd{anomaly features }\DIFaddend followed by low-rank models under multilinear transformations.

\subsubsection{Pattern Recognition for \DIFdelbegin \DIFdel{ASDW}\DIFdelend \DIFaddbegin \DIFadd{Anomaly Features Detection}\DIFaddend }
...
A classical model employed in many real-world applications is the (multidimensional) low-rank model (LRM) []\DIFdelbegin \DIFdel{. The model }\DIFdelend \DIFaddbegin \DIFadd{, which }\DIFaddend assumes that data have approximately low intrinsic dimensionality, e.g. lie either on some low-dimensional subspace or manifold [15,46], or is sparse in some basis [13]. Formally, LRM states that \DIFdelbegin \DIFdel{any measurement }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend tensor $\mathten{X}\in\mathbb{R}^{I_1\times \cdots I_N}$ can be decomposed as $\mathten{X} = \mathten{L} + \mathten{N}$, where $\mathten{L}\in\mathbb{R}^{I_1\times \cdots I_N}$ is a low-rank tensor, and $\mathten{N}\in\mathbb{R}^{I_1\times \cdots I_N}$ a residual tensor, a problem that \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{can be }\DIFaddend posed as an optimization problem, \DIFaddbegin \DIFadd{and is }\DIFaddend commonly solved by minimizing the \DIFaddbegin \DIFadd{Frobenius }\DIFaddend norm of $\mathten{N}$ as Equation \ref{eq:lowrankmodel} shows.
...
However, the brittleness of LRM with respect to grossly corrupted observations, often causes non optimal solutions []. For this reason, it is very common to replace \ref{eq:lowrankmodel} by the robust model $\mathten{X} = \mathten{L} + \mathten{S} + \mathten{N}$, called Robust LRM (RLRM), which is formulated as the \DIFdelbegin \DIFdel{minimization }\DIFdelend \DIFaddbegin \DIFadd{optimization }\DIFaddend problem of Equation \DIFdelbegin \DIFdel{\ref{eq:lowrankmodel2}}\DIFdelend \DIFaddbegin \DIFadd{\ref{eq:rlrmodel}}\DIFaddend , where $\left\lVert \cdot \right\rVert_\star$ denotes the tensor nuclear norm, $\left\lVert \cdot \right\rVert_1$ the $L_1$-norm, and $\mathten{S}\in\mathbb{R}^{I_1\times \cdots I_N}$ is a sparse tensor which models grossly corruptions. It should be notice that this decomposition model can be consider as a generalization of Equation \ref{eq:lowrankmodel}, since in the case of free-corruption, Equation \DIFdelbegin \DIFdel{\ref{eq:lowrankmodel2} }\DIFdelend \DIFaddbegin \DIFadd{\ref{eq:rlrmodel} }\DIFaddend reduces to \ref{eq:lowrankmodel}.

\begin{equation}
\DIFdelbegin \DIFdel{\min_{\mathten{L,S}} }%DIFDELCMD < {\left\lVert%%%
\DIFdel{\mathten{L}}%DIFDELCMD < \right\rVert%%%
\DIFdel{_\star + }%DIFDELCMD < \left\lVert%%%
\DIFdel{\mathten{S}}%DIFDELCMD < \right\rVert%%%
\DIFdel{_1}%DIFDELCMD < }
%DIFDELCMD < \label{eq:lowrankmodel2}
%DIFDELCMD < %%%
...
Although there exist many related works that successfully solve these models [15, 34], the RLRM lacks optimality when the intrinsic structure of a tensor is of high-rank, a situation that often happen in practice, which leads to a global solution that is not exactly low-rank.  However, \DIFdelbegin \DIFdel{recently Li, C., et. al, }\DIFdelend \DIFaddbegin \DIFadd{recent studies based on matrix LRM }\DIFaddend [\DIFdelbegin \DIFdel{x}\DIFdelend ] \DIFdelbegin \DIFdel{, showed that a low-rank based matrix completion problem }\DIFdelend \DIFaddbegin \DIFadd{show that we }\DIFaddend can obtain better performance by exploiting the \DIFaddbegin \DIFadd{intrinsic }\DIFaddend low-rank structure \DIFdelbegin \DIFdel{resulted after transforming an observed }\DIFdelend \DIFaddbegin \DIFadd{which results after transforming a }\DIFaddend matrix $\mathmat{X}\in\mathbb{R}^{m \times n}$ into a new representation $\mathmat{Y}\in\mathbb{R}^{m \times n}$ \DIFdelbegin \DIFdel{by applying a set of $K$ linear transformations $\{\mathcal{Q}_i(\cdot)\}_{i=1}^K$, ideas that established the foundations for a framework called }\DIFdelend \DIFaddbegin \DIFadd{either using linear or non-linear transformations. For second-order tensors (matrices), there exist some models that exploit the intrinsic low-rank structure of the transformed representation such as Structured Total Least Norm (STLN) }[]\DIFadd{, and kernel PCA }[]\DIFadd{. Furthermore, recently Li, C., et. al, }[\DIFadd{x}]\DIFadd{, study a LRM-based }\DIFaddend Matrix Completion under Multiple linear Transformations (MCMT). \DIFdelbegin \DIFdel{MCMT is formulated as an optimization problem (see Equation \ref{eq:mcmtmodel}), where $\mathcal{P}_\Omega(\cdot)$ denotes a downsampling operation over the supporting set $\Omega$, and $\delta\in\mathbb{R}^+$ a small constant}\DIFdelend \DIFaddbegin \DIFadd{Following these ideas, we propose a Tensor RLRM under Multilinear Transformation (TRLRMMT).
%DIF > which aims to exploit the induced multidimensional low-rank structures of the transformed space. 
}

\DIFadd{We formulate the TRLRMMT problem as finding a low-rank approximation of a tensor $\mathten{X}$ 
...
\DIFadd{$*********$NOTA$*********$ Para el problema MCMT, considera la estructura de bajo rango de la transformacion porque lo mete en el problema de optimizacion dentro de la norma, aqui tambien lo hacemos? A mi parecer si, puesto que la restriccion se termina introduciendo en la funcion de lagrange aumentada (sin restricciones), mas no en las normas, cosa contraria a MCMT}\DIFaddend .


\begin{equation}
	\DIFdelbegin %DIFDELCMD < \begin{aligned}
%DIFDELCMD < 		\min_{\mathmat{X}} \quad & {\sum_{i=1}^{K} \left\lVert \mathcal{Q}_i(\mathmat{X}) \right\rVert_*}\\
%DIFDELCMD < 		\textrm{s.t.} \quad & \left\lVert \mathcal{P}_\Omega(\mathmat{X}) - \mathcal{P}_\Omega(\mathmat{Y}) \right\rVert_F < \delta
%DIFDELCMD < 	\end{aligned}%%%
\DIFdelend \DIFaddbegin \begin{aligned}
		\min_{\mathten{L,S}} \quad & {\left\lVert \mathten{L} \right\rVert_\star + \left\lVert\mathten{S}\right\rVert_1}\\
		\textrm{s.t.} \quad & \Phi(\mathten{X}) = \mathten{L} + \mathten{S}
	\end{aligned}\DIFaddend 
\DIFdelbegin %DIFDELCMD < \label{eq:mcmtmodel}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{eq:horlrmmt}
\DIFaddend \end{equation}


\DIFdelbegin \DIFdel{Following the ideas behind MCMT on matrices, we propose the high-order RLRM under Multilinear Transformations (HoRLRM$^2$T) with applications }\DIFdelend \DIFaddbegin \DIFadd{$*********$ESTO IRÍA EN RESULTADOS?$*********$.
}

\DIFadd{Here, we employ the TRLRMMT }\DIFaddend for modeling and approximation of \DIFdelbegin \DIFdel{functions, where we exploit the additional multi-mode low-rank structures of the transformed tensors. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Reduccion de la complejidad de sistemas}\DIFdelend \DIFaddbegin \DIFadd{features }[]\DIFadd{, and system complexity reduction }[]\DIFadd{. For that purpose, we will assume that the behavior of any vehicle geometric feature over time can be modeled by either polynomial or rational functions. Then, we employ the ideas of }[\DIFadd{5}] \DIFadd{combined with the HoRLRM$^2$T to model the VTF tensor $\mathten{T}^{(N)}$ as either polynomial or rational functions through the Hankelization or L}{\DIFadd{\"o}}\DIFadd{wnerization transformations respectively, while reducing the transformed space complexity using RLRM. Finally, we recover the original domain applying the inverse transformation previously employed}\DIFaddend .


\DIFdelbegin \DIFdel{APARTE DE ESTE MODELO, REDACTAR CON EL ARTICULO DE "STRUCTURED TOTAL LEAST SQUARES" ASÍ COMO KERNEL PCA}\DIFdelend %DIF > First, we will assume that the behavior of any vehicle geometric feature over time can be modeled by either polynomial or rational functions. Then, by mapping the VTF tensor $\mathten{T}^{(N)}$ as a structured Hankel or L{\"o}wner tensor, we approximate the transformed space as a low-rank tensor using the t-SVD. Finally, we recover $\mathten{T}^{(N)}$ applying the inverse transformation.
\DIFaddbegin 



%DIF > which can be useful, e.g., to reduce a system complexity.

%DIF > However, recently Li, C., et. al, [x], showed that a low-rank based matrix completion problem can obtain better performance by exploiting the low-rank structure resulted after transforming an observed matrix $\mathmat{X}\in\mathbb{R}^{m \times n}$ into a new representation $\mathmat{Y}\in\mathbb{R}^{m \times n}$ by applying a set of $K$ linear transformations $\{\mathcal{Q}_i(\cdot)\}_{i=1}^K$, ideas that established the foundations for a framework called Matrix Completion under Multiple linear Transformations (MCMT). MCMT is formulated as an optimization problem (see Equation \ref{eq:mcmtmodel}), where $\mathcal{P}_\Omega(\cdot)$ denotes a downsampling operation over the supporting set $\Omega$, and $\delta\in\mathbb{R}^+$ a small constant.

%DIF > \begin{equation}
%DIF > 	\begin{aligned}
%DIF > 		\min_{\mathmat{X}} \quad & {\sum_{i=1}^{K} \left\lVert \mathcal{Q}_i(\mathmat{X}) \right\rVert_*}\\
%DIF > 		\textrm{s.t.} \quad & \left\lVert \mathcal{P}_\Omega(\mathmat{X}) - \mathcal{P}_\Omega(\mathmat{Y}) \right\rVert_F < \delta
%DIF > 	\end{aligned}
%DIF > \label{eq:mcmtmodel}
%DIF > \end{equation}


%DIF > We will also emphasize on reducing the system complexity
...
\subsection{Modeling and Approximation of \DIFdelbegin \DIFdel{Functions}\DIFdelend \DIFaddbegin \DIFadd{Features}\DIFaddend }
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Conclusions}
\DIFdelbegin \DIFdel{CONCLUSIONES DEL LOW-RANK Y TRANSFORMACIONES, }\DIFdelend \DIFaddbegin \textbf{\DIFadd{CONCLUSIONES DEL LOW-RANK Y TRANSFORMACIONES}}\DIFadd{. }\DIFaddend CUANDO CONOCEMOS LA FORMA ALGEBRAICA DE LAS CARACTERISTICAS SE EMPLEA HANKEL+T-SVD, DE OTRO MODO LOEWNER + T-RPCA, EL USO DE ESTOS MODELOS DE BAJO RANGO NOS AYUDAN A ELIMINAR DISTURBIOS EN LOS DATOS, \DIFdelbegin \DIFdel{CON LO CUAL SE PUEDE MEJORAR LOS MODELOS DE CLASIFICACION Y OCLUSION}\DIFdelend \DIFaddbegin \DIFadd{LO QUE SE TRADUCE EN UNA REDUCCION DE LA COMPLEJIDAD DE LOS DATOS, UNA MENOR COMPLEJIDAD EN LAS FUNCIONES DE CLASIFICACION (SIZE, OCCLUSION)}\DIFaddend .
\\
\\
\DIFdelbegin \DIFdel{TRABAJO FUTURO: 
1. USO DE OTRAS TRANSFORMACIONES MULTILINEALES COMO ESTOCASTICAS, SEGMENTATION, ASI  COMO NO LINEALES (KERNELS)
	}\DIFdelend \DIFaddbegin \textbf{\DIFadd{TRABAJO FUTURO}}\DIFadd{: 
}\begin{enumerate}[leftmargin=*,labelsep=4.9mm]
	\item 	\DIFadd{Uso de otras transformaciones multilineales como lo es la }\textit{\DIFadd{Segmentation}}\DIFadd{, }\textit{\DIFadd{Stochastic}}\DIFadd{, etc.
	}\item 	\DIFadd{Transformaciones no lineales (}\textit{\DIFadd{Kernel t-RPCA}}\DIFadd{)
	}\item	\DIFadd{Descomposiciones tensoriales con restricciones estructuradas (}\textit{\DIFadd{Hankel}}\DIFadd{, }\textit{\DIFadd{Loewner}}\DIFadd{, etc.): "}\textit{\DIFadd{Structured Tensor Decompositions}}\DIFadd{" a fin de explotar las propiedades de las estructuras de una manera mas eficientes (Articulo: }\textit{\DIFadd{Exploiting efficient representations in large-scale tensor decompositions}}\DIFadd{)}\DIFaddend .
	\DIFdelbegin \DIFdel{2. USO DE LAS DESCOMPOSICIONES TENSORIALES CON RESTRICCIONES ESTRUCTURADAS (HANKEL, LOEWNER, ETC), ASI COMO OTRAS RESTRICCIONES (NO NEGATIVIDAD)STRUCTURED TENSOR DECOMPOSITIONS.
	3. EMPLEO DEL MODELADO DE LOS DATOS PARA LA DETECCION DE OCLUSIONES.
}\DIFdelend \DIFaddbegin \item	\DIFadd{Empleo de nuestro modelo tensorial para la deteccion de oclusiones (Nuestro Articulo).
}\end{enumerate}
\DIFaddend 
